...
@contextmanager
def upload_model(
    bucketfs_location: AbstractBucketFSLocation, model_name: str, model_dir: Path
) -> Path:
    model_path = bucketfs_operations.get_model_path(model_params.sub_dir, model_name)
...
@contextmanager
def upload_model_to_local_bucketfs(model_name: str, download_tmpdir: Path) -> str:
    ...
    bucketfs_location = LocalFSMockBucketFSLocation(PurePosixPath(upload_tmpdir_name))
...
def upload_base_model_to_local_bucketfs(tmpdir_factory) -> PurePosixPath:
    ...
    with upload_model_to_local_bucketfs(model_params.base_model, tmpdir) as path:
        yield path
...
def upload_seq2seq_model_to_local_bucketfs(tmpdir_factory) -> PurePosixPath:
    ...
    with upload_model_to_local_bucketfs(model_params.seq2seq_model, tmpdir) as path:
        yield path


@contextmanager
def upload_model_to_bucketfs(
    model_name: str, download_tmpdir: Path, bucketfs_location: AbstractBucketFSLocation
) -> str:
    download_model(model_name, download_tmpdir)
    with upload_model(bucketfs_location, model_name, download_tmpdir) as model_path:
        try:
            yield model_path
        finally:
...
@pytest.fixture(scope="session")
def upload_base_model_to_bucketfs(bucketfs_location, tmpdir_factory) -> PurePosixPath:
    tmpdir = tmpdir_factory.mktemp(model_params.base_model)
    with upload_model_to_bucketfs(
        model_params.base_model, tmpdir, bucketfs_location
    ) as path:
        yield path
...
def upload_seq2seq_model_to_bucketfs(
    bucketfs_location, tmpdir_factory
) -> PurePosixPath:
    tmpdir = tmpdir_factory.mktemp(model_params.seq2seq_model)
    with upload_model_to_bucketfs(
        model_params.seq2seq_model, tmpdir, bucketfs_location
    ) as path:
        yield path
