...
class TestEnvironmentSetup:
    ...
    def __init__(self, id: str, url_localfs: str, token_conn_name: str):
        ...
        self.bucketfs_connection = Connection(
            address=f"{url_localfs}/bucket{id}", user=None, password=None
        )
        self.token_connection = (
            None
            if not self.token_conn_name
    ...
    def list_files_in_bucketfs(self):
        return self.bucketfs_location.list_files_in_bucketfs(str(self.sub_dir))


def test_model_downloader_udf_implementation():
    with tempfile.TemporaryDirectory() as tmpdir_name:
        url_localfs = f"file://{tmpdir_name}/bucket"
        env1 = TestEnvironmentSetup("1", url_localfs, token_conn_name="")
        env2 = TestEnvironmentSetup("2", url_localfs, token_conn_name="token_conn_name")
        ...
        assert (
            ctx.get_emitted()[0]
            == (str(env1.model_path), str(env1.model_path.with_suffix(".tar.gz")))
            and ctx.get_emitted()[1]
            == (str(env2.model_path), str(env2.model_path.with_suffix(".tar.gz")))
            and str(Path(ctx.get_emitted()[0][1]).relative_to(env1.sub_dir))
            in env1_bucketfs_files
            and str(Path(ctx.get_emitted()[1][1]).relative_to(env2.sub_dir))
            in env2_bucketfs_files
        )
