...
def test_model_upload(setup_database, pyexasol_connection, tmp_path: Path,
    ...
    model_specification_string = ModelSpecificationString(model_name=model_name)
    download_path = download_model_to_standard_local_save_path(model_specification_string, tmp_path)
    upload_path = bucketfs_operations.get_bucketfs_model_save_path(
        sub_dir, model_specification_string)
