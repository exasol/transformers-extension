...
from tests.unit_tests.udf_wrapper_params.token_classification.mock_sequence_tokenizer import (
...
class MockTokenClassificationModel:
    def __init__(
        ...
        self.result = [
            ...
            for start, end, word, entity, score in zip(
                starts, ends, words, entities, scores
            )
        ]
...
class MockTokenClassificationFactory:
    def __init__(self, mock_models: Dict[PurePosixPath, MockTokenClassificationModel]):
...
class MockPipeline:
    ...
    def __call__(
        self, text_data: List[str], aggregation_strategy: str
        ...
        return [result_list] * len(text_data) if len(text_data) > 1 else result_list
