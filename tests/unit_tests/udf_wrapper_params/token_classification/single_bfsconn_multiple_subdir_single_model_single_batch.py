...
class SingleBucketFSConnMultipleSubdirSingleModelNameSingleBatch:
    ...
    work_with_span_output_data = [("bfs_conn1", "sub_dir1", "model1", agg_strategy,
                    "text1", "label1", 0.1, token_docid, start+token_start, start+token_end, None)] * n_entities * data_size + \
                                 [("bfs_conn1", "sub_dir2", "model1", agg_strategy,
                                   "text1", "label2", 0.1, token_docid, start + token_start, start + token_end, None)] * n_entities * data_size
    ...
    mock_factory = MockTokenClassificationFactory({
        ...
        PurePosixPath(base_cache_dir1, "sub_dir2", "model1_token-classification"):
            MockTokenClassificationModel(
                starts=[0] * n_entities,
                ends=[6] * n_entities,
                words=["text1"] * n_entities,
