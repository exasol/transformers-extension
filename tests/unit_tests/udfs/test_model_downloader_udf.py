...
from exasol_transformers_extension.utils.huggingface_hub_bucketfs_model_transfer import (
    HuggingFaceHubBucketFSModelTransfer,
    HuggingFaceHubBucketFSModelTransferFactory,
    ModelFactoryProtocol,
)
from tests.unit_tests.utils_for_udf_tests import (
...
def test_model_downloader(
    description, count, token_conn_name, token_conn_obj, expected_token
):
    ...
    mock_model_downloader_factory: Union[
        HuggingFaceHubBucketFSModelTransferFactory, MagicMock
    ] = create_autospec(HuggingFaceHubBucketFSModelTransferFactory)
    mock_model_downloaders: List[
        Union[HuggingFaceHubBucketFSModelTransfer, MagicMock]
    ] = [create_autospec(HuggingFaceHubBucketFSModelTransfer) for i in range(count)]
    ...
    mock_bucketfs_factory: Union[BucketFSFactory, MagicMock] = create_autospec(
        BucketFSFactory
    )
    mock_bucketfs_locations = [Mock() for i in range(count)]
    mock_cast(
        mock_bucketfs_factory.create_bucketfs_location
    ).side_effect = mock_bucketfs_locations
    ...
    input_data = [
        (base_model_names[i], sub_directory_names[i], bfs_conn_name[i], token_conn_name)
        for i in range(count)
    ]
    ...
    for i in range(count):
        ...
        ).mock_calls == [call(mock_base_model_factory), call(mock_tokenizer_factory)]
        assert (
    ...
    assert mock_cast(
        ...
        [call(url=f"file:///test{i}", user=None, pwd=None) for i in range(count)]
    )
