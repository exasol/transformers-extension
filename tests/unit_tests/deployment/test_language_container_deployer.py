...
def mock_bfs_location(container_bfs_path) -> BucketFSLocation:
    ...
    mock_loc.generate_bucket_udf_path.return_value = PurePosixPath(
        f"/buckets/{container_bfs_path}"
    )
    return mock_loc
...
def container_deployer(
    mock_pyexasol_conn, mock_bfs_location, language_alias
) -> LanguageContainerDeployer:
...
def test_slc_deployer_deploy(
    container_deployer, container_file_name, container_file_path
):
    ...
    container_deployer.activate_container.assert_called_once_with(
        container_file_name, LanguageActivationLevel.System, True
    )
...
def test_slc_deployer_activate(
    container_deployer, container_file_name, container_file_path
):
    container_deployer.run(
        bucket_file_path=container_file_name, alter_system=True, allow_override=True
    )
...
@patch(
    "exasol_transformers_extension.deployment.language_container_deployer.get_language_settings"
)
def test_slc_deployer_generate_activation_command(
    ...
    command = container_deployer.generate_activation_command(
        container_file_name, alter_type
    )
...
def test_slc_deployer_generate_activation_command_failure(
    mock_lang_settings, container_deployer, language_alias, container_file_name
):
    current_bfs_path = "bfsdefault/default/container_abc"
    mock_lang_settings.return_value = (
        "R=builtin_r JAVA=builtin_java PYTHON3=builtin_python3 "
