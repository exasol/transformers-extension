...
def test_model_upload_python_api(tmpdir_factory):
    ...
    actual_tar_path = upload_model_to_bfs_location(model_name=model_specs.model_name,
                                                   task_type=model_specs.task_type,
                                                   subdir=sub_dir,
                                                   bucketfs_location=mock_bucketfs_location)

    expected_tar_path = sub_dir / (model_specs.get_model_specific_path_suffix()).with_suffix(".tar.gz")
