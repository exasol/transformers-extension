...
def udf_wrapper():
    ...
    from test.unit.udf_wrapper_params.filling_mask.token_conn_and_no_token_mixed_single_batch_complete import (
        TokenConnAndNoTokenMixedSingleBatchComplete as params,
    )
...
class TokenConnAndNoTokenMixedSingleBatchComplete:
    """
    ...
    input_data = [
        (None, "bfs_conn1", "token_conn1", "sub_dir1", "model1", "text <mask> 1", top_k)
    ] * data_size + [
        (None, "bfs_conn1", None, "sub_dir1", "model1", "text <mask> 1", top_k)
    ] * data_size
    output_data = [
        (
            "bfs_conn1",
