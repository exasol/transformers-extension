...
from test.unit.udf_wrapper_params.token_classification.make_data_row_functions import make_input_row, \
    ...
    text_doc_id, text_start, text_end, agg_strategy_simple, make_model_output_for_one_input_row,  \
    make_number_of_strings, sub_dir, model_name
...
class ErrorOnPredictionMultipleModelMultipleBatch:
    ...
    output_data = (make_output_row(bucketfs_conn=bfs_conn1, sub_dir=subdir1, model_name=model1) * n_entities *
                   data_size +
    ...
    work_with_span_input_data = (make_input_row_with_span(bucketfs_conn=bfs_conn1, sub_dir=subdir1, model_name=model1) *
                                 data_size +
                                 make_input_row_with_span(bucketfs_conn=bfs_conn2, sub_dir=subdir2, model_name=model2,
                                                          text_data="error on pred") * data_size)

    work_with_span_output_data1 = make_output_row_with_span(bucketfs_conn=bfs_conn1, sub_dir=subdir1,
                                                            model_name=model1) * n_entities * data_size
    # error on pred -> only one output per input
    work_with_span_output_data2 = [(bfs_conn2, subdir2, model2, text_doc_id, text_start, text_end, agg_strategy_simple,
                                   None, None, None, None, None, None,"Traceback")] * 1 * data_size
    ...
    # error on pred -> only one output per input
    tokenizer_model_output_df_model2_batch1 = [[Exception("Traceback mock_pipeline is "
                                                          "throwing an error intentionally")]]
    # error on pred -> only one output per input
    tokenizer_model_output_df_model2_batch2 = [[Exception("Traceback mock_pipeline is "
                                                          "throwing an error intentionally")]]

    tokenizer_models_output_df = [tokenizer_model_output_df_model1, tokenizer_model_output_df_model2_batch1,
                                  tokenizer_model_output_df_model2_batch2]
