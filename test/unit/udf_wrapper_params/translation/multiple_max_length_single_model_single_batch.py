...
def udf_wrapper():
    ...
    from exasol_transformers_extension.udfs.models.translation_udf import \
        TranslationUDF
    from test.unit.udf_wrapper_params.translation. \
...
class MultipleMaxLengthSingleModelNameSingleBatch:
    ...
    output_data = [("bfs_conn1", "sub_dir1", "model1", "text 1", src_lang,
                    target_lang,  max_length1, "text 1 übersetzt" * max_length1, None)
                   ] * data_size + \
                  [("bfs_conn1", "sub_dir1", "model1", "text 1", src_lang,
                    target_lang,  max_length2, "text 1 übersetzt" * max_length2, None)
                   ] * data_size
