...
from test.unit.udf_wrapper_params.ai_extract_extended.make_data_row_functions import (
    bucketfs_conn,
    entity_type,
    make_input_row,
...
class MultipleModelMultipleBatchCompleteMultipleEntities:
    """
    Multiple models, multiple batches, last batch complete
    """
    ...
    input_data = (
        make_input_row(sub_dir=sub_dir1, model_name=model_name1, text_data=text_data1)
        * data_size
        + make_input_row(sub_dir=sub_dir2, model_name=model_name2, text_data=text_data2)
        * data_size
    )
    output_data = (
        make_output_row(
    ...
    work_with_span_input_data = (
        make_input_row_with_span(
            sub_dir=sub_dir1, model_name=model_name1, text_data=text_data1
        )
        * data_size
        + make_input_row_with_span(
            sub_dir=sub_dir2, model_name=model_name2, text_data=text_data2
        )
        * data_size
    )
