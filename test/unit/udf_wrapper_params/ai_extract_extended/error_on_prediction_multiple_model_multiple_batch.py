...
class ErrorOnPredictionMultipleModelMultipleBatch:
    ...
    input_data = (
        make_input_row(bucketfs_conn=bfs_conn1, sub_dir=subdir1, model_name=model1)
        * data_size
        + make_input_row(
    ...
    output_data = (
        make_output_row(bucketfs_conn=bfs_conn1, sub_dir=subdir1, model_name=model1)
        * n_entities
        * data_size
        + make_output_row(
            bucketfs_conn=bfs_conn2,
    ...
    work_with_span_input_data = (
        make_input_row_with_span(
            bucketfs_conn=bfs_conn1, sub_dir=subdir1, model_name=model1
        )
        * data_size
        + make_input_row_with_span(
            bucketfs_conn=bfs_conn2,
    ...
    work_with_span_output_data = (
        work_with_span_output_data1 + work_with_span_output_data2
    )
    ...
    # error on pred -> only one output per input
    tokenizer_model_output_df_model2_batch1 = [
