...
class SingleBucketFSConnMultipleSubdirSingleModelNameMultipleBatch:
    """
    Single BucketFS connection, multiple subdirs, single model, multiple batches
    """
    ...
    input_data = (
        make_input_row(sub_dir=sub_dir1) * data_size
        + make_input_row(sub_dir=sub_dir2) * data_size
    )
    output_data = (
        make_output_row(sub_dir=sub_dir1, answer=answer1, score=score) * data_size
        + make_output_row(sub_dir=sub_dir2, answer=answer2, score=score + 0.1)
        * data_size
    )

    work_with_span_input_data = (
        make_input_row_with_span(sub_dir=sub_dir1) * data_size
        + make_input_row_with_span(sub_dir=sub_dir2) * data_size
    )
    work_with_span_output_data = (
        make_output_row_with_span(sub_dir=sub_dir1, answer=answer1, score=score)
        * data_size
