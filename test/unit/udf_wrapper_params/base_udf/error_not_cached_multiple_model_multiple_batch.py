...
class ErrorNotCachedMultipleModelMultipleBatch:
    ...
    input_data = (
        make_input_row(bucketfs_conn=bfs_conn1, sub_dir=subdir1) * data_size
        + make_input_row(
            bucketfs_conn=bfs_conn2, sub_dir=subdir2, model_name="non_existing_model"
        )
        * data_size
    )

    output_data = (
        make_output_row(bucketfs_conn=bfs_conn1, sub_dir=subdir1) * data_size
        + make_output_row(
    ...
    work_with_span_input_data = (
        make_input_row_with_span(bucketfs_conn=bfs_conn1, sub_dir=subdir1) * data_size
        + make_input_row_with_span(
            bucketfs_conn=bfs_conn2, sub_dir=subdir2, model_name="non_existing_model"
        )
        * data_size
    ...
    work_with_span_output_data = (
        make_output_row_with_span(bucketfs_conn=bfs_conn1, sub_dir=subdir1) * data_size
        + make_output_row_with_span(
            bucketfs_conn=bfs_conn2,
            sub_dir=subdir2,
            model_name="non_existing_model",
