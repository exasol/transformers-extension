...
class ReturnMixedMultipleModelMultipleBatchComplete:
    ...
    bfs_connections = {bucketfs_conn: Connection(address=f"file://{base_cache_dir}")}
    ...
    outputs_single_text_batch_1 = (
        output_single_text_row_1 * data_size + output_single_text_row_2 * data_size
    )
    outputs_single_text_batch_2 = (
        output_single_text_row_3 * data_size + output_single_text_row_4 * data_size
    )
    outputs_single_text = outputs_single_text_batch_1 + outputs_single_text_batch_2

    model_output_single_text_one_batch = [
        make_model_output_for_one_input_row() * batch_size
    ]
    sequence_models_output_df_single_text = [
        model_output_single_text_one_batch + model_output_single_text_one_batch
    ]
    ...
    outputs_text_pair_batch_1 = (
        output_text_pair_row_1 * data_size + output_text_pair_row_2 * data_size
    )
    outputs_text_pair_batch_2 = (
        output_text_pair_row_3 * data_size + output_text_pair_row_4 * data_size
    )
    outputs_text_pair = outputs_text_pair_batch_1 + outputs_text_pair_batch_2

    model_output_text_pair_one_batch = [
        make_model_output_for_one_input_row() * batch_size
    ]
    sequence_models_output_df_text_pair = [
        model_output_text_pair_one_batch + model_output_text_pair_one_batch
    ]
