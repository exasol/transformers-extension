...
@pytest.mark.parametrize(
    ...
    [
        (
            "add all",
            pd.DataFrame(data),
            ["model_name","sub_dir","bucketfs_conn","device_id","top_k","return_ranks","max_new_tokens","return_full_text","aggregation_strategy"],
            (3,12),
            "None",
        ...
        (
                "add some",
                pd.DataFrame(data),
                ["model_name","sub_dir","bucketfs_conn","device_id","top_k","return_ranks"],
                (3, 9),
                "None",
                "model_for_a_specific_udf"
        ),
        (
                "add to empty df",
                pd.DataFrame(),
                ["model_name", "sub_dir", "bucketfs_conn", "device_id", "top_k", "return_ranks", "max_new_tokens",
                 "return_full_text", "aggregation_strategy"],
                (0, 10),#the dataframe columns will be created, but no rows exist so no rows will be filled
                "None",
                "model_for_a_specific_udf"
        ),
        (
...
def test_add_default_columns_transformation(
    ...
    for output_df in output_generator:
        ...
        if "model_name" in default_cols:
            assert all(output_df["model_name"][i] == DEFAULT_MODEL_SPECS[udf_name].model_name for i in output_df.index)
            default_cols.remove("model_name")
        assert all(output_df[def_col][i] == DEFAULT_VALUES[def_col] for def_col in default_cols for i in output_df.index)
...
def test_add_wrong_default_columns_transformation(
    ...
    for output_df in output_generator:
        ...
        assert all(def_col in output_df.columns for def_col in default_cols)
        assert all(output_df[def_col][i] is None for def_col in default_cols for i in output_df.index)
        assert output_df.shape == expected_dataframe_shape
