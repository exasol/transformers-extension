...
def test_parameter_formatters_2params():
    ...
    formatters = _ParameterFormatters()
    formatters.set_formatter(
        CustomizableParameters.container_url,
        "http://my_server/{version}/{user}/my_stuff",
    )
    formatters.set_formatter(
        CustomizableParameters.container_name, "downloaded-{version}"
    )
