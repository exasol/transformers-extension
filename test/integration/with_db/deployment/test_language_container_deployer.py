...
from test.fixtures.database_connection_fixture import pyexasol_connection
from test.fixtures.language_container_fixture import (
    export_slc,
    flavor_path,
)
from test.utils.parameters import bucketfs_params
...
def test_language_container_deployer(
    ...
    with revert_language_settings(pyexasol_connection):
        ...
        deployer = create_container_deployer(
            language_alias=language_alias,
            pyexasol_connection=pyexasol_connection,
            bucketfs_config=bucketfs_config,
        )
        deployer.run(
            container_file=container_path, alter_system=True, allow_override=True
        )
    ...
    with revert_language_settings(pyexasol_connection):
        ...
        with connection_factory(exasol_config) as new_connection:
            ...
            deployer.activate_container(
                container_path.name, LanguageActivationLevel.Session, True
            )
            assert_udf_running(new_connection, language_alias, schema)
    ...
    with revert_language_settings(pyexasol_connection):
        ...
        with connection_factory(exasol_config) as new_connection:
            ...
            with pytest.raises(RuntimeError):
                deployer.activate_container(
                    container_path.name, LanguageActivationLevel.System, False
                )
...
def assert_udf_running(
    pyexasol_connection: ExaConnection, language_alias: str, schema: str
):
