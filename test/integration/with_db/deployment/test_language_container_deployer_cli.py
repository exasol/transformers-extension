...
def call_language_definition_deployer_cli(
    dsn: str,
    ...
    if use_ssl_cert_validation:
        args_list += ["--use-ssl-cert-validation"]
    else:
        args_list += ["--no-use-ssl-cert-validation"]
...
def test_language_container_deployer_cli_with_container_file(
    ...
    with revert_language_settings(pyexasol_connection):
        ...
        assert (
            result.exit_code == 0 and result.exception == None and result.stdout == ""
        )
        assert_udf_running(
