...
def test_download_pretrained_model():
    ...
    with tempfile.TemporaryDirectory() as download_tmpdir:
        download_model_to_standard_local_save_path(
            model_params.tiny_model_specs, download_tmpdir
        )
        url_fields = []
        ...
        assert Path(download_tmpdir).is_dir() and all(
            model_params.tiny_model_specs.model_name in url_ for url_ in url_fields
        )
