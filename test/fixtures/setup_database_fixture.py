...
def _create_bucketfs_connection(
    bucketfs_config: config.BucketFs, pyexasol_connection: ExaConnection
) -> None:
    parsed_url = urlparse(bucketfs_config.url)
    host = parsed_url.netloc.split(":")[0]
    address = (
        f"{parsed_url.scheme}://{host}:{bucketfs_params.real_port}/{bucketfs_params.bucket}/"
        f"{bucketfs_params.path_in_bucket};{bucketfs_params.name}"
    )
    query = (
        f"CREATE OR REPLACE  CONNECTION {bucketfs_connection_name} "
        f"TO '{address}' "
