...
class ScriptsDeployer:
    ...
    def _deploy_udf_scripts(self) -> None:
        for udf_call_src, template_src in constants.UDF_CALL_TEMPLATES.items():
            udf_content = constants.UDF_CALLERS_DIR.joinpath(udf_call_src).read_text()
            ...
            logger.debug(
                f"The UDF statement of the template " f"{template_src} is executed."
            )
    ...
    def run(
        ...
        websocket_sslopt = utils.get_websocket_ssl_options(
            use_ssl_cert_validation, ssl_cert_path
        )
