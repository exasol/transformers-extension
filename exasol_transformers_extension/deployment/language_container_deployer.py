...
def get_websocket_sslopt(
    ...
    if ssl_client_certificate:
        if not Path(ssl_client_certificate).is_file():
            raise ValueError(
                f"Certificate file {ssl_client_certificate} doesn't exist."
            )
...
class LanguageContainerDeployer:
    ...
    def download_and_run(
        ...
        with tempfile.NamedTemporaryFile() as tmp_file:
            ...
            self.run(
                Path(tmp_file.name), bucket_file_path, alter_system, allow_override
            )
    ...
    def run(
        ...
        if not bucket_file_path:
            if not container_file:
                raise ValueError(
                    "Either a container file or a bucket file path must be specified."
                )
    ...
    def get_language_definition(self, bucket_file_path: str):
        ...
        """
        path_in_udf = self._bucketfs_location.generate_bucket_udf_path(bucket_file_path)
        result = self._generate_new_language_settings(
            path_in_udf=path_in_udf, prev_lang_aliases=[]
        )
        return result
