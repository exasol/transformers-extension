...
class _ParameterFormatters:
    ...
    def __call__(
        ...
        def update_parameter(parameter_name: str, formatter: str) -> None:
            ...
            if param_formatter:
                ...
                param_formatter = re.sub(
                    pattern, lambda m: f"{{{m.group(0)}}}", param_formatter
                )
    ...
    def set_formatter(
        self, custom_parameter: CustomizableParameters, formatter: str
    ) -> None:
...
@click.option(
    "--container-file", type=click.Path(exists=True, file_okay=True), default=None
)
@click.option(
    "--version",
    type=str,
...
@click.option(
    "--use-ssl-cert-validation/--no-use-ssl-cert-validation", type=bool, default=True
)
