...
from exasol_transformers_extension.utils.model_specification import (
    create_model_specs_from_path,
)
...
class ListModelsUDF:
    ...
    def _parse_model_info_from_path(
        self, model_paths_list: set[str], sub_dir: str, bfs_conn_name: str
    ) -> None:
        for model_path in model_paths_list:
            try:
                model_spec, warning = create_model_specs_from_path(
                    pathlib.Path(model_path), sub_dir
                )
                if warning:
